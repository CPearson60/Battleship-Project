...
def player_shoot():
    while True:
        ...
        if len(shot) == 2 and shot[0].lower() in [list] and shot[1].isdigit() and int(shot[1]) <= col_list:
            break
        else:
...
#Function For Computer To Randomly Generate Cohord To Shoot
def comp_shoot(alphabet_lower):
    while True:
    ...
    X = "x"
    shipPlaceDisplayGrid[row][col] = f"{Fore.LIGHTYELLOW_EX}{X}{Fore.LIGHTCYAN_EX}"
    coordGrid[row][col] = X
    return (shipPlaceDisplayGrid, coordGrid, X)
...
def manual_ship_place(alphabet_lower):
    ...
    while True:
        ...
        if n>4:
            ...
            battleGrid(shipPlaceDisplayGrid)
            time.sleep(1)
            comp_shoot(alphabet_lower)
            time.sleep(1)
